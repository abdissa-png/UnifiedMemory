...
**`GraphRetriever`** uses **graph store** operations (and may combine with vector signals depending on implementation) for multi-hop / PPR-style behavior—see `retrieval/graph.py` and graph store protocol.
